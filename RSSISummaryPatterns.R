...
recordName <- "24-phoneb"
summaryFile <- paste(folder,"/",recordName,"-summary-filtered.csv",sep="")
regFile <- paste(folder,"/",recordName,"-regressions.csv",sep="")
#intercept <- -50
#coefficient <- -11
...
regressions <- read.table(regFile,sep=",",header=TRUE, stringsAsFactors=FALSE)
intercept <- regressions$intercept[1]
coefficient <- regressions$coefficient[1]
...
rplot <- ggplot(summary, aes(x=distsquared, y=modediff, color=3)) +
  ...
  stat_cor(label.x = 0.1, label.y = 14) + 
  stat_regline_equation(label.x = 0.1, label.y = 12) +
