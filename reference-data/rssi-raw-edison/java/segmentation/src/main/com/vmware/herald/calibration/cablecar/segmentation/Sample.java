...
public class Sample {
	...
	public void add(final Sample distribution) {
		if (n == 0) {
			...
			combined.m4 = m4 + distribution.m4
					+ delta4 * n * distribution.n * (n * n - n * distribution.n + distribution.n * distribution.n)
							/ (combined.n * combined.n * combined.n);
			combined.m4 += 6.0 * delta2 * (n * n * distribution.m2 + distribution.n * distribution.n * m2)
					/ (combined.n * combined.n)
					+ 4.0 * delta * (n * distribution.m3 - distribution.n * m3) / combined.n;
	...
	/// Estimate distance between this sample's distribution and another sample's
	/// distribution, 1 means identical and 0 means completely different.
	public Double distance(final Sample sample) {
		return bhattacharyyaDistance(this, sample);
	}

	/// Bhattacharyya distance between two distributions estimate the likelihood
	/// that the two distributions are the same.
	/// bhattacharyyaDistance = 1 means the two distributions are identical; value =
