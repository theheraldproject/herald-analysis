...
public class DefaultHandler extends AbstractHttpHandler {
	...
	private final static String[] helpText = new String[] {
			...
			"",
			"GET:  /status?csv",
			"  Get status of all connected devices in CSV format",
			"",
			...
			"",
			"POST: /broadcast?commands=[A]&id=[B]",
			"  Broadcast command separated commands [A] to device with id [B] as soon as possible. Supported commands are:",
			"  Server will respond with the list of all scheduled commands in JSON format.",
			"",
			...
			"",
			"POST: /broadcast?id=[A]&clear",
			"  Clear all pending commands for device with id [B].",
			"  Server will respond with the list of all scheduled commands in JSON format.",
			"",
